...
function TeacherPerformance({ quizzes }: { quizzes: TeacherQuiz[] }) {
  const totalEngagement = quizzes.reduce((sum, q) => sum + q.totalParticipants, 0);
  const avgEngagement = quizzes.length > 0 ? Math.round(totalEngagement / quizzes.length) : 0;
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-gradient-to-br from-blue-500/20 to-blue-600/10 p-4 rounded-lg border border-blue-500/30 hover:border-blue-400/50 transition">
          ...
          <p className="text-3xl font-black text-blue-300 mt-2">{totalEngagement}</p>
          <p className="text-xs text-slate-500 mt-2">📈 students participated</p>
        </div>
      ...
      <div>
        ...
        <div className="space-y-3">
          {quizzes.map((quiz, index) => {
            const maxValue = 50;
            const percentage = Math.min((quiz.totalParticipants / maxValue) * 100, 100);
...
function StudentPerformance({ quizzes }: { quizzes: StudentQuiz[] }) {
  const avgScore = quizzes.length > 0 ? Math.round(quizzes.reduce((sum, q) => sum + q.bestScore, 0) / quizzes.length) : 0;
  const topScore = quizzes.length > 0 ? Math.max(...quizzes.map(q => q.bestScore)) : 0;
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="space-y-3">
          {quizzes.map((quiz, index) => {
            const percentage = Math.min((quiz.bestScore / 100) * 100, 100);
